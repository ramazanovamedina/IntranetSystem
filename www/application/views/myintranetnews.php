<head>
	...
	<script type = "text/javascript">
		...
		$("button#submit").click(function(){
			var data = $("#secret").val();
			$("#example").modal('hide');
			location.href = "/intranetnews/delete?id="+data;
		});
	});
	</script>
</head>
		...
		<table class="table table-bordered">
			...
			<?php foreach($mynews as $mynew):?>
				<tr>
					...
					<td><a href="<?php echo URL::site('intranetnews/details/'.$mynew->news_id); ?>"><?php echo $mynew->content_short; ?></a></td>
					<td><?php echo $mynew->post_date; ?></td>
						<td><a href = "#" onclick = "del(<?php echo $mynew->news_id?>)">Удалить</a></td>
					<td><a href="<?php echo URL::site('intranetnews/update?id='.$mynew->news_id);?>">Исправить</a></td>
				</tr>		
			<?php endforeach; ?>	
		</table>
	<?php }else{ ?>
		<div class="alert alert-info">
			У Вас нет опубликованных Вами статей
		</div>
	<?php } ?>
