...
function getMonthYearString(dt) {
  return ['Январь','Февраль','Март','Апрель','Май','Июнь','Июль',
          'Август','Сентябрь','Октябрь','Ноябрь','Декабрь'][dt.getMonth()] +
         ' ' + dt.getFullYear();
}
...
function createCalendar(div, month) {
  ...
  daysRow.insertCell(-1).innerHTML="Пн";
  daysRow.insertCell(-1).innerHTML="Вт";
  daysRow.insertCell(-1).innerHTML="Ср";
  daysRow.insertCell(-1).innerHTML="Чт";
  daysRow.insertCell(-1).innerHTML="Пт";
  daysRow.insertCell(-1).innerHTML="Сб";
  daysRow.insertCell(-1).innerHTML="Вс";
